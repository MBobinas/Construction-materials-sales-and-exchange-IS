    ...
    <body>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    {{ Breadcrumbs::render('user.activeTrade.index') }}
                </li>
            </ol>
            @if (session('success'))
        ...
        <div class="container-fluid">
            <div class="col-md-11 text-center">
                ...
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{ route('user.activeTrade.index') }}" class="btn btn-outline-primary active"
                        aria-current="page">Gauti Pasiūlymai</a>
                    <a href="{{ route('user.activeTrade.sentTrades') }}" class="btn btn-outline-primary">Išsiųsti
                        pasiūlymai</a>
                </div>
            </div>
                    ...
                    @foreach ($trades as $trade)
                        @if ($trade->status == 'laukiama' || $trade->status == 'priimta' || $trade->status == 'atšaukta')
                            <div class="row p-2 bg-white border rounded">
                                ...
                                <div class="align-items-center align-content-center col-3 col-md-3 col-sm-3 border-left mt-1">
                                    <div class="d-flex flex-row align-items-center">
                                        <ul class="ms-5 list-group list-group-flush">
                                            <li class="list-group-item">
                                    ...
                                    @if ($trade->status == 'laukiama')
                                        <div class="mt-2 text-md-center text-sm-center">
                                            <div class="btn-group">
                                                <form action="{{ route('user.trade.accept', $trade->id) }}" method="POST"
                                                    onsubmit="return submitForm(this);">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="btn btn-success">Priimti pasiūlymą</button>
                                                </form>
                                                <form action="{{ route('user.trade.cancel', $trade->id) }}" method="POST"
                                                    onsubmit="return submitCancel(this);">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="ms-2 btn btn-danger">Atmesti
                                                        pasiūlymą</button>
                                                </form>
                                            </div>
                                            <div class="text-center">
                                                <span class="badge bg-warning text-dark">Laukiama pasiūlymo patvirtinimo</span>
                                            </div>
                                        </div>
                                    ...
                                    @elseif($trade->status == 'priimta')
                                        ...
                                        <div class="mt-2 text-center">
                                            ...
                                            <div class="text-center">
                                                <span class="badge bg-success text-wrap">Pasiūlymas priimtas, laukiama mainų
                                                    pabaigos</span>
                                            </div>
                                        </div>
                                    @elseif($trade->status == 'atšaukta')
                                        <div class="mt-2 text-md-end text-sm-end">
                                            ...
                                            <div class="text-center">
                                                <span class="badge bg-danger text-wrap"> Pasiūlymas atmestas, laukiama
                                                    pašalinimo.</span>
                                            </div>
                                        </div>
                                    @endif
        ...
        <script>
            function submitForm(form) {
                Swal.fire({
                    title: "Ar tikrai norite priimti pasiūlymą?",
                    icon: "question",
                    buttons: true,

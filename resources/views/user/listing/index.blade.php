...
@section('content')
    ...
    <body>
        {{ Breadcrumbs::render('user.listings.show', $listing) }}
        @if (session('error'))
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-8">
                    <h2 class="text-left text-break text-wrap " style="color:#28282B">
                        {{ $listing->title }}
                        <hr>
                    </h2>
                </div>
                <div class="col-sm-4">
                    <div class="row">
                        ...
                        <div class="col-sm-9 text-start">
                            ...
                            @endif
                            <span>Skelbimas patalpintas: {{ $listing->created_at->format('m/d/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-7 border border-dark rounded" style="background-color: #DBE0E7">
                    ...
                    <div class="col-sm-12">
                        @if ($listing->listing_type == 'parduoti')
                            <form action="{{ route('user.listings.checkout', $listing->id) }}"
                                        ...
                                        <table class="table align-items-center mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($products as $product)
                                                    ...
                                                    @if($product->min_quantity == 0 || $product->min_quantity == null)
                                                        <td class="align-middle text-center text-sm"><span
                                                            class="text-secondary text-xs font-weight-normal">Nėra
                                                            min. kiekio</span></td>
                                                    @else
                                ...
                                <div class="form-group mt-2">
                                    <button name="submit" type="submit" class="btn btn-primary">Pirkti</button>
                                </div>
                            </form>
                        @endif

                        @if ($listing->listing_type == 'mainyti')
                            <form action="{{ route('user.trade.index', $listing->id) }}" enctype="multipart/form-data"
                                ...
                                <div class="form-group mt-2">
                                    <a href="{{ route('user.trade.index', $listing->id) }}"><button name="submit"
                                            type="submit" class="btn btn-primary">Mainyti</button></a>
                                </div>
                            </form>
                        @endif
                        ...
                        @if ($listing->listing_type == 'parduoti arba mainyti')
                            <form action="{{ route('user.listings.checkout', $listing->id) }}"
                                        ...
                                        <table class="table align-items-center mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($products as $product)
                                                    ...
                                                    @else
                                                        <td class="align-middle text-center text-sm"><span
                                                            class="text-secondary text-xs font-weight-normal">{{ $product->min_quantity }}</span>
                                                        <span
                                                            class="text-secondary text-xs font-weight-normal">{{ $product->measurment_unit }}</span>
                                                        </td>
                                                    @endif
                                ...
                                <div class="form-group mt-2">
                                    <button name="submit" type="submit" class="btn btn-primary">Pirkti</button>
                                    <button name="submit" type="submit" class="btn btn-primary">Mainyti</button>
                                </div>
                            </form>
                        @endif
                        ...
                        <div class="col-md-5">
                            <div class="card">
                                <div class="card-body p-3">
                                            ...
                                            @else
                                                @foreach ($comments as $comment)
                                                        ...
                                                        <div class="flex-grow-1 flex-shrink-1">
                                                            <div class="border-bottom">
                                                                <div
                                                                    ...
                                                                    <form action="/skelbimas/{{ $listing->id }}/komentaras/{{ $comment->id }}" method="POST" 
                                                                        onsubmit="return submitDelete(this);">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit" class="btn btn-link text-danger">
                                                                            <i class="fa-light fa-x" title="Pašalinti"></i>
                                                                        </button>
                                                                        {{-- <a href="/skelbimas/{{ $listing->id }}/komentaras/{{ $comment->id }}"
                                                                            title="Pašalinti">
                                                                            <i class="fa-light fa-x"></i></a> --}}
                                                                    @endif
                                                                </div>
                                                                <p class="small mb-0">
                                                                    {{ $comment->text }}
                                                    ...
                                                    <!-- reply part of comment
                                                            Kolkas palieku taip, poto jeigu laiko liks reply functionality
                                                            ...
                                                            <div class="flex-grow-1 flex-shrink-1">
                                                                <div>
                                                                    <div
                                                                        class="d-flex justify-content-between align-items-center">
                                                                        <p class="mb-1">
                                                                            Matas Bobinas <span class="small">- Prieš
                                                                                3 val.</span>
                                                                        </p>
                                                                    </div>
                                                                    <p class="small mb-0">
                                                                        Sveiki, turime dar lentų. Rašykite į el. paštą.
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div> -->
                                    ...
                                    <form action="{{ route('user.listings.comment', $listing->id) }}" method="POST"
                                        onsubmit="return submitForm(this);">
                                        @csrf

                                        <div class="row mt-1">
                                            <h5 class="text-center" style="color:#28282B;">Palikti komentarą</h5>
                                            <div class="col-md-12">
                                            ...
                                            <div class="col-md-12 border rounded p-2 mb-1" id="test">
                                                <textarea class="form-control textarea" id="comment" onkeyup="validate()" placeholder="Rašykite komentarą..." rows="2"
                                        ...
                                        <div class="d-flex flex-row justify-content-end mt-1">
                                            <input class="btn btn-primary float-right" type="submit" id="submit_comment"
                                                value="Komentuoti">
                                        </div>
                                    </form>
                                </div>
        ...
        <script>
            function submitForm(form) {
                Swal.fire({
                        title: "Ar tikrai talpinti komentarą?",
                        text: "Jūsų komentaras bus matomas visiems naudotojams.",
                        icon: "question",
                        buttons: true,
                        dangerMode: true,
                        ...
                        confirmButtonText: 'Talpinti komentarą',
                        cancelButtonText: 'Atšaukti'
                    })
                    .then(function(result) {
                        if (result.isConfirmed) {
                            Swal.fire({
                                title: 'Komentaras patalpintas sėkmingai!',
                                icon: 'success',
                            ...
                            Swal.fire({
                                title: "Komentaras nepatalpintas",
                                text: "Jūsų komentaras nepatalpintas.",
                                icon: "error",
                                confirmButtonColor: '#198754',
...
<script>
    function submitDelete(form) {
        Swal.fire({
                title: "Ar tikrai norite pašalinti komentarą?",
                text: "Jūsų komentaras bus pašalintas.",
                icon: "error",
                buttons: true,
                dangerMode: true,
                showCancelButton: true,
                confirmButtonColor: '#198754',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Šalinti komentarą',
                cancelButtonText: 'Atšaukti'
            })
            .then(function(result) {
                if (result.isConfirmed) {
                    Swal.fire({
                        title: 'Komentaras pašalintas sėkmingai!',
                        icon: 'success',
                        showConfirmButton: false,
                    });
                    form.submit();

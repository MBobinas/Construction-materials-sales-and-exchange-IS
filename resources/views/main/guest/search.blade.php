    ...
    <body style="background-color: #F0F2F5">
        <nav>
            <div class="nav-wrapper"> 
                <div class="ms-2">
                    {{ Breadcrumbs::render('main.guest.search') }}     
                </div>    
                <div class="col-md-2 col-sm-2 mx-2">
                    <form id="search" action="/svečias/paieška" method="GET">
                        @csrf
                        <div class="input-group input-group-outline ms-2">
                            <input type="text" class="form-control" placeholder="Įveskite ieškomo skelbimo pav." name="search"
                                id="search" onfocus="focused(this)" onfocusout="defocused(this)">
                        </div>
                    </form>
                </div>
                <div class="ms-3 mt-2">
        ...
        <div class="container-fluid my-2">
            <div class="d-flex row">
                ...
                <div class="col-md-2 bg-gradient-dark border rounded">
                            ...
                            <div class="accordion-item h-100">
                                <button class="accordion-header">
                                    <a href="{{ route('main.guest.filter.category', $category->category_name) }}"
                                        class="btn btn-link text-white" style="text-decoration: none">{{ $category->category_name }}
                                    </a>
                                    ...
                                    @foreach ($category->children as $child)
                                        <div class="accordion-body">
                                            <a href="{{ route('main.guest.filter.category', $child->category_name) }}"
                                                class="btn btn-link text-white" style="text-decoration: none">{{ $child->category_name }}
                                            </a>
                                        </div>
                                    @endforeach
                                @endif
